...
export function ListHeroes({ attribute, heroes, onHeroClick, className }: ListHeroesProps) {
  const { attributeLabel, attributeIconSrc, attributeColor } = useListHeroes(attribute);

  return (
    <section className={cn("w-full", className)} aria-label={`${attributeLabel} heroes`}>
      <header className="mb-2 flex items-center gap-2">
        <Image
          ...
          unoptimized
        />
        <h3 className={cn("text-sm font-bold uppercase tracking-wide", attributeColor)}>
          {attributeLabel}
        </h3>
      </header>
