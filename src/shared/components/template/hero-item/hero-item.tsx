...
export function HeroItem({
  ...
  return (
    <button
      ...
      onClick={handleClick}
      className={cn(
        "group relative h-20 w-18 overflow-hidden border border-border bg-background text-foreground transition duration-150 ease-in-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/70",
        className,
      )}
    ...
    >
      <Image
        ...
        width={90}
        height={80}
        className={cn("h-20 w-18 object-cover transition duration-150 ease-in-out", imageStateClass)}
        onError={handleError}
        unoptimized
